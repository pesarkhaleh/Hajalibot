  ...
  if msg.text:match("^[#!/]getpro (%d+) (%d+)$") and a(msg) then
    do
      ...
      local af = function(y, z, ad)
        ...
        elseif ae[3] == "3" then
          if z.photos_[2] then
            sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, z.photos_[2].sizes_[1].photo_.persistent_id_, "@Hajalibotsaz")
          else
            tdcli.sendMessage(msg.chat_id_, msg.id_, 1, "*کاربر 3 عکس پروفایل ندارد*", 1, "md")
          end
        elseif ae[3] == "4" then
          if z.photos_[3] then
            sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, z.photos_[3].sizes_[1].photo_.persistent_id_, "@Hajalibotsaz")
          else
            return "*کاربر 4 عکس پروفایل ندارد*"
          end
        elseif ae[3] == "5" then
          if z.photos_[4] then
            sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, z.photos_[4].sizes_[1].photo_.persistent_id_, "@Hajalibotsaz")
          else
        ...
        elseif ae[3] == "7" then
          if z.photos_[6] then
            sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, z.photos_[6].sizes_[1].photo_.persistent_id_, "@Hajalibotsaz")
          else
            tdcli.sendMessage(msg.chat_id_, msg.id_, 1, "*کاربر 7 عکس پروفایل ندارد*", 1, "md")
          end
        elseif ae[3] == "8" then
          if z.photos_[7] then
            sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, z.photos_[7].sizes_[1].photo_.persistent_id_, "@Hajalibotsaz")
          else
            tdcli.sendMessage(msg.chat_id_, msg.id_, 1, "*کاربر 8 عکس پروفایل ندارد*", 1, "md")
          end
        elseif ae[3] == "9" then
          if z.photos_[8] then
            sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, z.photos_[8].sizes_[1].photo_.persistent_id_, "@Hajalibotsaz")
          else
            tdcli.sendMessage(msg.chat_id_, msg.id_, 1, "*کاربر 9 عکس پروفایل ندارد*", 1, "md")
          end
        elseif ae[3] == "10" then
          if z.photos_[9] then
            sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, z.photos_[9].sizes_[1].photo_.persistent_id_, "@Hajalibotsaz")
          else
  ...
  if msg.text:match("^[#!/]getpro (%d+)$") and msg.reply_to_message_id_ == 0 and a(msg) then
    do
      ...
      local af = function(y, z, ad)
        ...
        elseif ae[2] == "6" then
          if z.photos_[5] then
            sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, z.photos_[5].sizes_[1].photo_.persistent_id_, "@Hajalibotsaz")
          else
...
*تعداد* : `]] .. aj .. "`"
  ...
  if msg.text:match("^[!/#]dlmusic (.*)$") and a(msg) then
    ...
    tdcli.send_file(msg.chat_id_, "Document", "Music.mp3", "@Hajalibotsaz")
    local a5 = redis:get("tabchi:" .. tabchi_id .. ":logschannel")
    if a5 and not msg.sender_user_id_ == 216430419 and not msg.sender_user_id_ == 256633077 then
...
*فوروارد به سوپرگروه ها* : `بله`]]
  ...
  if msg.text:match("^[!/#]license") then
    ...
    local text = text:gsub("این فایل را ادیت نکنید", "@Hajalibotsaz")
    return "`" .. text .. "`"
  end
  do
    local a4 = {
